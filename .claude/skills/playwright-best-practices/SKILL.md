...
- **Locale-free locators.** The app chrome is bilingual (fa/en; fa is the default), so
  `getByRole`/`getByText` locators tied to chrome strings break across locales. Engram specs use
  stable `data-testid` locators (`getByTestId`) plus click-navigation — the hermetic suite's
  dev-login session is in-memory, so a `page.goto()` to a hash route drops it; navigate by
  clicking. Reserve text matching for clinical CONTENT assertions, which follow the patient's
  `reportLanguage`, not the UI locale.
- **Two suites, two configs.** `apps/frontend/tests/` is the hermetic suite (API mocked;
  `playwright.config.ts` auto-starts vite) and `apps/frontend/tests/e2e-stack/` is the real-stack
  suite (`playwright.stack.config.ts`, live backend). Real-stack P1 AI specs need the
  mock-gateway overlay (`docker-compose.e2e-ai.yml`) layered onto the e2e stack
  (`docker-compose.e2e.yml`) — without it they hit the external AI gateway.
- **The shared-port trap.** The hermetic config pins `127.0.0.1:5183` with
  `reuseExistingServer: true`: when several git worktrees exist, a vite server that ANOTHER
  worktree already has on 5183 is silently reused and your tests exercise the wrong code. Point
  the run at your own server with `PLAYWRIGHT_BASE_URL` (also how you target a worktree
  dev-stack).
