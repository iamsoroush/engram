...
description: >-
  ...
  effect from a vague description.
---
...
- **IS NOT:** choosing overall visual direction, palettes, or typography (in Engram those are owned by `docs/design-principles.md` and the `docs/ux/` docs), auditing a whole page's UI quality, or named text-effect specs.
...
- **RTL first.** Engram's chrome is bilingual (fa/en) and fa — the primary locale — is RTL. The
  directional-motion rules above are written LTR ("forward" = left-to-right): under RTL,
  horizontal motion mirrors — forward is right-to-left, and panels/drawers emerge from the
  mirrored edge. Express horizontal motion in logical terms (inline-start/inline-end, `:dir()`
  or a `[dir="rtl"]` flip) rather than hardcoded left/right, and check both directions.
  Vertical motion and opacity/scale are direction-neutral. Language/RTL model:
  `docs/frontend/i18n.md`.
- **Visual direction is already decided.** Palettes, type, and overall look are owned by
  `docs/design-principles.md` + `docs/ux/`; this skill governs motion implementation only. In a
  clinical utility app, default to restraint — high-frequency capture actions stay instant.
