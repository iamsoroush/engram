...
type Lang = "fa" | "en";
const FA = { recent: "اخیر", patients: "بیماران", needsInput: "توجه لازم" };
...
async function chromeLeaks(page: import("@playwright/test").Page) {
  return page.evaluate(() => {
    ...
    const words = ["Recent", "Yesterday", "This week", "Patients", "Needs input", "Attention", "Clinical Memory", "All caught up", "No patients", "Search patients", "Loading", "Searching"];
...
test.describe("S4 Clinical Memory — fa Persian + RTL, data verbatim, en unchanged, core journey", () => {
  ...
  test("fa: memory tabs are Persian; no English chrome leak (scan excludes [data-content])", async ({ page }) => {
    ...
    await expect(page.getByText(FA.recent).first()).toBeVisible();
