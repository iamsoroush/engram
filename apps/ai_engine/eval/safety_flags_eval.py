...
def _label_problem(kind: str, matches: list[dict[str, Any]], want: Any) -> str | None:
    """Check the AES-1801 `label` gate over a kind's flags: return a failure note, or None if any is good.
...
def run_gates(output: dict[str, Any], case: dict[str, Any]) -> list[str]:
    ...
    for wanted in case["expect"]:
        ...
        # AES-1801 label gate: the normalized short label is the legible primary — assert it exists and is
...
GATE_SELF_TESTS: list[dict[str, Any]] = [
    ...
    # --- AES-1801 label gate self-tests (the legible primary must be normalized/native/grounded) ---
    {
