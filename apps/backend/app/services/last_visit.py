...
def get_session_context(
    ...
    **ordinal**. The Basic context card renders this directly; the Pro window layers Job-4 intelligent
    ...
    return {
        ...
        "keyFacts": key_facts,
        # Cross-visit clinical safety flags (allergy/contraindication/consent) from PRIOR visits'
        # synthesis — surfaced prominently at the point of care. The in-progress visit is excluded so
        # its own flags show only in the opt-out "this visit" panel, never duplicated here as history.
        "safetyFlags": patient_safety_flags_payload(patient, exclude_session_id=exclude_session_id),
    }
