...
edits); postgres `engram-postgres-1` (`engram/engram`, host 5442); minio `engram-minio-1`
(`engram-dev`/`engram-dev-secret`, host 9010, bucket `engram-captures`); redis. **Gotchas:** the
ai-engine Celery worker has **no autoreload** — `docker restart engram-ai-engine-1` after editing
`apps/ai_engine`; new alembic migrations only run on backend container start, so run
`docker exec engram-backend-1 sh -c "cd /app && alembic upgrade head"` after adding one.
...
5. **Dev tooling — make both tiers testable side-by-side** (Epic A1) — **DONE (2026-06-04).**
   ...
   `DEV_TENANT_BASIC_ID` (Basic, "Engram Demo Clinic (Basic)", own "Bita B." seed patient) — with
