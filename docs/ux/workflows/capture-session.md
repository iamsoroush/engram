...
12. Users can rename or delete a capture from the capture item settings menu (Pro also offers a one-tap `Undo last capture` on the Sources drawer). Removal is de-effecting: it reverts the capture's patient action, treatments, safety flags, and report contribution, restoring or recomputing the report (see [capture.md](../screens/capture.md) "Sources drawer and undo").
13. The report is the session's **primary surface** — there is **no Generate button** and no separate report tab to "unlock"; it is built/synthesized automatically as captures land. Before the first capture exists, the report area shows guidance to capture first.
14. Adding, deleting, or reassigning captures re-runs organization automatically. For **Pro**, the prior synthesized report **stays visible** with an "Updating · N captures…" line while the new synthesis runs (it never blanks to a draft); **Basic** rebuilds its deterministic live report in place. The report surface is described in [capture.md](../screens/capture.md). Session save (`POST /sessions/{id}/save`) is a pipeline detail auto-invoked via the outbox as captures sync — never a user action.
