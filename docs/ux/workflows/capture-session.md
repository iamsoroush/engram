...
12. Users can rename or delete a capture from the capture item settings menu. Deleting a capture removes it and the report re-organizes automatically; deletion also de-effects that capture's patient action (see [redesign-capture-undo.md](../redesign-capture-undo.md)).
...
14. Adding, deleting, or reassigning captures re-runs organization automatically. For **Pro**, the prior synthesized report **stays visible** with an "Updating · N captures…" line while the new synthesis runs (it never blanks to a draft); **Basic** rebuilds its deterministic live report in place. The current report surface is authoritative in [capture.md](../screens/capture.md) (FB8 + live-report refinements).
