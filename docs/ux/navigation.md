...
| `/#qa-inbox` | Q&A inbox (Pro) | **Pro only** — reached from a top-bar inbox icon beside Search (not the primary nav pill); hidden on Basic (capability `post_session_qa`). The icon carries a **pending-thread count badge** (AES-1801 — a deliberate partial-revert of the E16 merge: messages earn their own glanceable badge again, while the unified **Attention indicator** keeps its merged Messages count). An **urgent** (red-flagged) question turns the badge to the danger tone. **Thread-centric**: one patient conversation per entry (chat bubbles + interleaved visit markers), threads awaiting approval first. Each pending question shows an AI-suggested reply the doctor can Send / edit / Dismiss (AES-402); `Mine`/`Clinic` scope, a routing-mode control, and **Re-route** to one of the patient's treating doctors. |
=======
| `/#patients` | Clinical Memory | Recent, Patients, and Attention tabs (Pro adds Lists). Recent is a time-bucketed recent-activity list (Active visit / Today / Yesterday / This week); the Attention tab is the severity-tiered [Close-the-day sweep](screens/patients.md#attention-tab). Patient rows open patient detail/timeline; the main view does not nest sessions under patients. |
...
- Top-bar **Attention indicator** (a severity-coloured bell beside Search): shown only when something is open; tapping it opens the Attention sweep. It keeps the merged needs-input + Q&A **Messages** count, but an **urgent** red-flagged patient question (AES-1801) escalates its tone above safety. Q&A also has its **own glanceable pending-thread badge** on the inbox icon (a deliberate partial-revert of the E16 merge). The Clinical-Memory hero **chip** shows the **same** attention count and opens the same sweep, so the two never disagree.
